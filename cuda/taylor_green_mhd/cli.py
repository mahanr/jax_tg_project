...
def main():
    parser = argparse.ArgumentParser(
        description="3D Taylor-Green MHD vortex with CuPy/CUDA"
    )
    parser.add_argument("--n", type=int, default=128)
    parser.add_argument("--dt", type=float, default=0.005)
    parser.add_argument("--reynolds", type=float, default=1000.0)
    parser.add_argument("--magnetic-reynolds", type=float, default=1000.0)
    parser.add_argument("--magnetic-amplitude", type=float, default=0.5)
    ...
    config = TaylorGreenMhdConfig(
        ...
        magnetic_reynolds=args.magnetic_reynolds,
        magnetic_amplitude=args.magnetic_amplitude,
        total_time=args.total_time,
        save_every_time=args.save_every_time,
    )
    TaylorGreenMhdSimulation(config, make_plots=not args.no_plots).run()
